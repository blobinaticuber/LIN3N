class Menu {
  ...
  void drawColourMenu() {
    ...
    for (int c = 0; c < puzzle.posColours.length; c++) {
      ...
      rect(2*width/3 + (c*width/3/puzzle.negColours.length), 100, width/3/puzzle.posColours.length, height/3/puzzle.posColours.length);
      fill(puzzle.negColours[c]);
      rect(2*width/3 + (c*width/3/puzzle.negColours.length), 200, width/3/puzzle.negColours.length, height/3/puzzle.negColours.length);
    }
  }
